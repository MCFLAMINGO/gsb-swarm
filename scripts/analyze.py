...
def generate_excel_report(
    ...
    """Generate a multi-tab Excel workbook with raw data + summary metrics."""
    import openpyxl
    ...
    DARK_TEAL  = "1B474D"
    LIGHT_TEAL = "BCE2E7"
    ACCENT     = "CC3311"  # bleeding.cash red
    LIGHT_GRAY = "F7F6F2"
    WHITE      = "FFFFFF"

    def header_cell(ws, row, col, value, bold=True, bg=DARK_TEAL, fg=WHITE):
    ...
    def value_cell(ws, row, col, value, fmt=None, red_if_negative=False):
        c = ws.cell(row=row, column=col, value=value)
        c.font = Font(size=10)
        if fmt:
            c.number_format = fmt
        if red_if_negative and isinstance(value, (int, float)) and value < 0:
            c.font = Font(size=10, color=ACCENT)
        return c
    ...
    header_cell(ws, 1, 1, f"{project_name} — Financial Summary")
    ws.merge_cells("A1:C1")
    ws.cell(row=2, column=1, value=f"Period: {period}").font = Font(italic=True, size=9)
    ws.cell(row=2, column=3, value="Generated by bleeding.cash").font = Font(italic=True, size=9, color="7A7974")

    rows = [
        ("REVENUE", None, None),
        ("Net Sales",           metrics.get("net_sales_total", 0),      "$#,##0.00"),
        ("Total Cash In",       metrics.get("total_cash_in", 0),         "$#,##0.00"),
        ("", None, None),
        ("EXPENSES", None, None),
        ("Total Cash Out",      metrics.get("total_cash_out", 0),        "$#,##0.00"),
        ("Food & Beverage",     metrics.get("food_cost_total", 0),       "$#,##0.00"),
        ("Labor",               metrics.get("labor_total", 0),           "$#,##0.00"),
        ("Debt Service",        metrics.get("debt_service_total", 0),    "$#,##0.00"),
        ("Other Expenses",      metrics.get("other_expenses_total", 0),  "$#,##0.00"),
        ("", None, None),
        ("CASH FLOW", None, None),
        ("Net Cash Flow",       metrics.get("net_cash_flow", 0),         "$#,##0.00"),
        ("Closing Balance",     metrics.get("closing_balance") or 0,     "$#,##0.00"),
        ("", None, None),
        ("RATIOS", None, None),
        ("Food Cost %",         metrics.get("food_cost_pct") or 0,       "0.00%"),
        ("Labor %",             metrics.get("labor_pct") or 0,           "0.00%"),
        ("Prime Cost %",        metrics.get("prime_cost_pct") or 0,      "0.00%"),
        ("", None, None),
        ("PERIODS", None, None),
        ("Months of Data",      metrics.get("num_months", 1),            "0"),
        ("Near-Zero Events",    len(metrics.get("near_zero_events", [])), "0"),
    ]
    ...
    for label, val, fmt in rows:
        if val is None and label:
            ...
            c.fill = PatternFill("solid", fgColor=DARK_TEAL)
            ws.merge_cells(f"A{r}:C{r}")
        elif val is not None:
            label_cell(ws, r, 1, label)
            is_pct = fmt and "%" in fmt
            display_val = val * 100 if is_pct else val
            value_cell(ws, r, 3, display_val, fmt, red_if_negative=True)
        r += 1
    ...
    # ═══════════════════════════════════════════════════════
    # TAB 2 — Monthly Cash Flow
    # ═══════════════════════════════════════════════════════
    ws2 = wb.create_sheet("Monthly Cash Flow")
    monthly = metrics.get("monthly_summary", {})

    header_cell(ws2, 1, 1, "Month")
    header_cell(ws2, 1, 2, "Cash In")
    header_cell(ws2, 1, 3, "Cash Out")
    header_cell(ws2, 1, 4, "Net Flow")
    header_cell(ws2, 1, 5, "Sales")

    if monthly:
        for i, (month, mdata) in enumerate(sorted(monthly.items()), start=2):
            ws2.cell(row=i, column=1, value=month)
            value_cell(ws2, i, 2, mdata.get("cash_in", 0),    "$#,##0.00")
            value_cell(ws2, i, 3, mdata.get("cash_out", 0),   "$#,##0.00")
            net = mdata.get("cash_in", 0) - mdata.get("cash_out", 0)
            value_cell(ws2, i, 4, net,  "$#,##0.00", red_if_negative=True)
            value_cell(ws2, i, 5, mdata.get("sales", 0),      "$#,##0.00")
    else:
        ws2.cell(row=2, column=1, value="No monthly breakdown available")
    ...
    if bank_df is not None and not bank_df.empty:
        ws3 = wb.create_sheet("Bank Transactions")
        cols = list(bank_df.columns)
        for ci, col in enumerate(cols, start=1):
            header_cell(ws3, 1, ci, str(col))
        for ri, (_, row) in enumerate(bank_df.iterrows(), start=2):
            for ci, col in enumerate(cols, start=1):
                val = row[col]
                ...
                if col == "amount" and isinstance(val, (int, float)) and val < 0:
                    c.font = Font(size=9, color=ACCENT)
                else:
                    c.font = Font(size=9)
        auto_col_width(ws3, min_w=8, max_w=35)

    # ═══════════════════════════════════════════════════════
    # TAB 4 — POS Sales (if available)
    # ═══════════════════════════════════════════════════════
    if pos_df is not None and not pos_df.empty:
        ws4 = wb.create_sheet("POS Sales")
        cols = list(pos_df.columns)
        for ci, col in enumerate(cols, start=1):
            header_cell(ws4, 1, ci, str(col))
        for ri, (_, row) in enumerate(pos_df.iterrows(), start=2):
            for ci, col in enumerate(cols, start=1):
                val = row[col]
                if hasattr(val, "strftime"):
                    val = val.strftime("%Y-%m-%d")
                    ...
                    val = val.item()
                ws4.cell(row=ri, column=ci, value=val).font = Font(size=9)
        auto_col_width(ws4, min_w=8, max_w=35)

    # ═══════════════════════════════════════════════════════
    # TAB 5 — Vendor Spend
    # ═══════════════════════════════════════════════════════
    vendor_breakdown = metrics.get("vendor_breakdown", {})
    if vendor_breakdown:
        ws5 = wb.create_sheet("Vendor Spend")
        header_cell(ws5, 1, 1, "Vendor / Description")
        header_cell(ws5, 1, 2, "Category")
        header_cell(ws5, 1, 3, "Total Paid")
        ri = 2
        for vendor, vdata in sorted(vendor_breakdown.items(), key=lambda x: -abs(x[1].get("total", 0))):
            ws5.cell(row=ri, column=1, value=vendor).font = Font(size=9)
            ws5.cell(row=ri, column=2, value=vdata.get("category", "")).font = Font(size=9)
            value_cell(ws5, ri, 3, vdata.get("total", 0), "$#,##0.00", red_if_negative=True)
            ri += 1
        auto_col_width(ws5)

    # ── Disclaimer tab ───────────────────────────────────────────────────────
    wsd = wb.create_sheet("Disclaimer")
    wsd.cell(row=1, column=1, value="DISCLAIMER").font = Font(bold=True, size=12)
    wsd.cell(row=3, column=1, value="This report is for informational purposes only and does not constitute financial,")
    wsd.cell(row=4, column=1, value="legal, or accounting advice. Always consult a licensed professional before making")
    wsd.cell(row=5, column=1, value="financial decisions. Operated by MCFL Restaurant Holdings LLC.")
    wsd.cell(row=7, column=1, value=f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    wsd.column_dimensions["A"].width = 80
...
def main():
    ...
    if args.mode == "personal":
        ...
        print("\n[4/4] Excel Data Export...")
        excel_path = str(output_dir / f"{slug}-financial-data-{ym}.xlsx")
        try:
            generate_excel_report(metrics, bank_df, pos_df, project_name, args.period, excel_path)
            generated.append(excel_path)
            print(f"  Excel saved: {excel_path}")
        except Exception as e:
            print(f"  [excel] WARNING: Could not generate Excel: {e}")
