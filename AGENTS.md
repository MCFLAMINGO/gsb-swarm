...
- Boot backfill: `scripts/backfillBusinesses.js` (also triggered from `index.js` when
  `LOCAL_INTEL_DB_URL` is set). Conflict target must match
  `idx_businesses_name_zip_unique` / `lib/db.js upsertBusiness`:
  `ON CONFLICT (lower(trim(name)), zip) WHERE sunbiz_doc_number IS NULL`.
- System packages (Postgres/pgvector) and the seeded DB are expected to persist via the
  environment snapshot. If a future VM starts without them: start Postgres
  (`sudo pg_ctlcluster 16 main start`), ensure `LOCAL_INTEL_DB_URL` is set, run migrations
  (boot `index.js` once or any path that calls `lib/dbMigrate.js`), then reseed via the
  boot backfill or by loading `data/zips/*.json` + `data/localIntel.json`.
