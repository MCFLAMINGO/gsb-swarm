 ...
 * Rail selection is handled by lib/railRouter.js (OpenRouter-style scorer).
 * ─────────────────────────────────────────────────────────────
...
async function releaseTempoEscrow(escrow_data, completion_note) {
  ...
  // Prefer settlementService when booking_id is present (real payout path).
  if (escrow_data.booking_id) {
    try {
      const settlementService = require('./settlementService');
      return await settlementService.settleOnComplete({
        booking_id: escrow_data.booking_id,
        completion_note,
      });
    } catch (e) {
      console.warn('[dispatchRail] settlementService release error:', e.message);
    }
  }
  console.log(`[dispatchRail] Tempo escrow release (legacy log): ${escrow_data.amount} ${escrow_data.token} → ${escrow_data.to} (booking: ${escrow_data.booking_id})`);
  return { released: true, token: escrow_data.token, amount: escrow_data.amount, status: 'settled_intent' };
}
...
/**
 * dispatchToBusiness — uses railRouter to select the best rail and fire.
 *
 * railRouter scores all rails for this business + transaction context,
 * picks the winner, falls back automatically on failure.
 *
 * Returns { rail, result, decision } or null if all rails fail.
 */
async function dispatchToBusiness(biz, rfq) {
  ...
  // Ask railRouter for the best rail
  const ctx      = {
    type:       rfq.budget_usd ? 'order' : 'rfq_fee',
    amount_usd: rfq.budget_usd || null,
  };
  const decision = await railRouter.selectRail(biz, ctx);

  console.log(`[dispatchRail] railRouter → ${decision.rail} (score=${decision.score} reason=${decision.reason} fallback=${decision.fallback || 'none'})`);

  // Helper: attempt a rail, return result or null
  async function tryRail(rail) {
    switch (rail) {
      case 'surge':     return dispatchSurge(biz, rfq, acceptUrl);
      case 'stripe':    return dispatchStripe(biz, rfq, acceptUrl);
      case 'sms':       return dispatchTwilio(biz, rfq, acceptUrl, declineUrl);
      case 'rfq':       return { rail: 'rfq', rfq_id: rfq.id }; // email handled by rfqService
      default:          return null;
    }
  }

  // Try winner first, then fallback
  let result = await tryRail(decision.rail);
  let usedRail = decision.rail;

  if (!result && decision.fallback) {
    console.log(`[dispatchRail] ${decision.rail} failed, trying fallback: ${decision.fallback}`);
    result   = await tryRail(decision.fallback);
    usedRail = decision.fallback;
  }

  if (!result) {
    // Last resort: rfq broadcast
    result   = { rail: 'rfq', rfq_id: rfq.id };
    usedRail = 'rfq';
  }
  ...
  return {
    rail:     usedRail,
    result,
    decision, // full scores + fee_destination for fee_events.meta
  };
}
