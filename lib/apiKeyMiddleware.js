...
// B72: Free-pass pattern — read tools and RFQ/job creation are FREE (no key required).
// Payment only fires on job acceptance/completion (Tempo pathUSD), not on search or RFQ ask.
// Only tools that involve wallet operations, payment settlement, or admin actions remain gated.
const FREE_TOOL_PATTERN = /(search|query|get|list|find|intel|intelligence|discover|zip|business|category|rfq_create|rfq_ask|rfq|create_rfq|post_job|task_create|oracle|signal|sector_gap|tide|zone|nearby|context|bedrock|corridor|changes|stats|realtor|healthcare|retail|construction|restaurant)/i;

function isFreeTool(body) {
  if (!body || body.method !== 'tools/call') return false;
  const name = body.params?.name || '';
  if (!name) return false;
  // Premium tools are explicitly gated regardless of name pattern
  if (PREMIUM_TOOLS.has(name)) return false;
  return FREE_TOOL_PATTERN.test(name);
}
...
function createApiKeyMiddleware(db) {
  ...
  return async function apiKeyMiddleware(req, res, next) {
    ...
    // 1b. B72 — tools/call for read-only or RFQ creation tools is FREE.
    // Payment now fires only on job acceptance/completion (Tempo pathUSD).
    // Wallet ops, settlement, and admin tools still fall through to the key check below.
    if (method === 'tools/call' && isFreeTool(body)) {
      return next();
    }
    ...
    try {
      ...
      // Fail closed on DB error — never grant paid access when we can't verify the token.
      // Discovery methods (tools/list, initialize, ping, notifications/*) have already
      // short-circuited above, so they remain free even when Postgres is flaky.
      return res.status(503).json({ error: 'Service temporarily unavailable', code: 'DB_ERROR' });
    }
