 ...
 */
function twiml(say, gather = null, redirect = null, opts = {}) {
  ...
  // B11: optional <Record> verb for transcript logging. Placed after Gather so it
  // only fires when the gather times out without speech — Twilio records the
  // remainder of the call and posts the recording + transcription to our webhooks.
  if (opts.record) {
    xml += `  <Record transcribe="true" ` +
           `transcribeCallback="/api/local-intel/voice/transcription-complete" ` +
           `recordingStatusCallback="/api/local-intel/voice/recording-complete" ` +
