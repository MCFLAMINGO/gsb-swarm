...
function twiml(say, gather = null, redirect = null) {
  ...
  if (gather) {
    ...
    xml += `    <Say voice="alice">${escXml(say)}</Say>\n`;
    xml += `  </Gather>\n`;
    // Fallback if no speech detected
    xml += `  <Say voice="alice">We didn't catch that. Please call back and try again.</Say>\n`;
  } else {
    xml += `  <Say voice="alice">${escXml(say)}</Say>\n`;
  }
  if (redirect) {
...
async function handleIncoming({ stage, speechResult, callerPhone }) {
  ...
  if (category || zip) {
    ...
    const xml = twiml(
      ...
      `${timing ? 'For ' + timing + '. ' : ''}` +
      `You'll receive a text with quotes shortly. Goodbye!`
    );
    return { contentType: 'text/xml', body: xml };
  }
