...
async function handleIncoming({ stage, speechResult, callerPhone }) {
  ...
  if (category || zip) {
    ...
    if (isUncertain) {
      ...
      // Be honest — we heard them but couldn't match a specific service
      responseText =
        `Thanks${name ? ', ' + name : ''}. I heard your request but I want to be upfront — ` +
        `I wasn't sure exactly which service you need. ` +
        `I've logged your request in ${resolvedZip} and our team will follow up with you. ` +
        `You can also visit the local intel dot com to browse available services. Goodbye!`;
    } else {
      responseText =
        `Got it${name ? ', ' + name : ''}! ` +
        `We're finding ${categoryLabel} providers${zip ? ' in ' + resolvedZip : ' near you'}. ` +
        `${timing ? 'For ' + timing + '. ' : ''}` +
        `Local businesses will be in touch shortly. Goodbye!`;
    }
