...
async function handleIncoming({ stage, speechResult, callerPhone, callSid }) {
  ...
  if (_taskSessionId) {
    ...
    if (pending) {
      ...
      if (allTasks.length > 1 && idx < allTasks.length) {
        ...
        if (nextIdx < allTasks.length) {
          ...
          const xml = twiml(`${msg} I just texted you — reply with the answer. Goodbye!`);
          return { contentType: 'text/xml', body: xml };
        }
        ...
        const xml = twiml(
          ...
          `You'll receive a text confirmation in a moment. Goodbye!`
        );
        return { contentType: 'text/xml', body: xml };
      }
    ...
    if (taskIntent && taskIntent.isTask) {
      ...
      const xml = twiml(
        `${taskIntent.followUp} I just sent you a text — reply with the answer and we'll handle it. Goodbye!`
      );
      return { contentType: 'text/xml', body: xml };
    }
  }
    ...
    if (!isUncertain) {
      try {
        ...
        if (surgeRows.length > 0) {
          ...
          // TwiML — tell caller to check their phone
          const xml = twiml(
