...
async function broadcastJob({ jobId, code, callerName, category, zip, description }) {
  ...
  if (zip) {
    providers = await db.query(
      `SELECT business_id, name, phone, notification_email AS email, agent_endpoint, agent_key
       FROM businesses
       WHERE status != 'inactive'
         AND (category ILIKE $1 OR category_group ILIKE $1)
         AND zip = $2
         AND (phone IS NOT NULL OR notification_email IS NOT NULL OR agent_endpoint IS NOT NULL)
       ORDER BY (agent_endpoint IS NOT NULL) DESC, (claimed_at IS NOT NULL) DESC, confidence_score DESC
       LIMIT 20`,
      [catLike, zip]
    );
  }
