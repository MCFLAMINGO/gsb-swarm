...
const agentBid    = require('./agentBid');

// ── Table migrations ──────────────────────────────────────────────────────────
let migrated = false;
async function migrate() {
  if (migrated) return;
  // Ensure agent columns exist on businesses
  await agentBid.migrate().catch(e => console.warn('[rfqBroadcast] agentBid.migrate warn:', e.message));
  await db.query(`
  ...
  `);
  // NOTE: v2 (rfqService) owns the `rfq_responses` table now with column `rfq_id`.
  // v1 (broadcast) uses its own separate table `rfq_broadcast_responses` to avoid
  // the column-name collision that caused `rfq-poll` to throw `column "job_id" does not exist`.
  await db.query(`
    CREATE TABLE IF NOT EXISTS rfq_broadcast_responses (
      id            BIGSERIAL PRIMARY KEY,
      job_id        UUID NOT NULL REFERENCES rfq_jobs(id) ON DELETE CASCADE,
      business_id   TEXT,
      business_name TEXT,
  ...
  await db.query(`CREATE INDEX IF NOT EXISTS rfq_broadcast_responses_job_idx ON rfq_broadcast_responses(job_id)`);
  migrated = true;
}
...
async function broadcastJob({ jobId, code, callerName, category, zip, description }) {
  ...
  // SMS GATE: business must have a wallet (paid participant) AND have opted into SMS.
  // wallet IS NOT NULL = they are a paying participant in the network.
  // notify_sms = TRUE  = they have pre-paid / subscribed to receive SMS job alerts.
  // Both conditions required — wallet alone is not enough.
  if (zip) {
    providers = await db.query(
      `SELECT business_id, name, phone, COALESCE(notification_email, contact_email) AS email,
              agent_endpoint, agent_key, dispatch_token
       FROM businesses
       ...
       ORDER BY (agent_endpoint IS NOT NULL) DESC, confidence_score DESC
       LIMIT 20`,
      [catLike, zip]
    );
  }
  ...
  // ── Agent-to-agent: fan out to providers that have an agent_endpoint ──────
  // Runs in parallel with SMS/email. Best bid logged; SMS/email still sent as fallback.
  const agentBidResult = await agentBid.broadcastAgentRfq(
    { id: jobId, code, category, zip, description, budget_usd: null, deadline_at: null, caller_phone: null },
    providers
  ).catch(e => {
    console.warn('[rfqBroadcast] agentBid error (non-fatal):', e.message);
    return { bids: [], selected: null, agent_count: 0 };
  });

  if (agentBidResult.agent_count > 0) {
    console.log(`[rfqBroadcast] Agent bids — ${agentBidResult.agent_count} agents contacted, ${agentBidResult.bids.length} accepted`);
  }

  const nq = require('./notificationQueue');
  let broadcastCount = 0;
  const notifiedProviders = [];

  for (const p of providers) {
    try {
      // Primary channel: email alert to business notification_email via notificationQueue.
      // SMS is never sent by LocalIntel — the business configures their own employee
      // notification preferences (notify_sms) independently in their admin settings.
      const subject = `New Job: ${catLabel}${zipStr} — ${code}`;
      const dashUrl = `https://www.thelocalintel.com/inbox.html?token=${p.dispatch_token || ''}`;
      await nq.enqueue(
        p.business_id,
        subject,
        {
          body:      `${description || `New ${catLabel} job request${zipStr}.`}`,
          job_code:  code,
          category:  catLabel,
          zip:       zip || null,
          cta_url:   dashUrl,
      ...
      await db.query(
        ...
        [jobId, p.business_id, p.name, p.phone || null, p.email || null]
      );
      broadcastCount++;
      notifiedProviders.push({ name: p.name, email: p.email || null });
      console.log(`[rfqBroadcast] Queued email alert for ${p.name}`);
    } catch (err) {
      console.warn(`[rfqBroadcast] Failed to notify ${p.name}:`, err.message);
    }
  }
  ...
  // Return both count AND provider list so callers can send customer confirmation
  return { count: broadcastCount, providers: notifiedProviders };
}
...
async function recordResponse({ code, providerPhone, providerEmail, channel, rawText }) {
  ...
  if (bizId) {
    const existing = await db.query(
      `SELECT id FROM rfq_broadcast_responses WHERE job_id = $1 AND business_id = $2`,
      [job.id, bizId]
    );
    if (existing.length) {
  ...
  // Insert response
  await db.query(
    `INSERT INTO rfq_broadcast_responses (job_id, business_id, business_name, business_phone, business_email, channel, raw_text)
...
async function confirmSelection({ jobId, responseIndex, callerPhone }) {
  ...
  const responses = await db.query(
    `SELECT * FROM rfq_broadcast_responses WHERE job_id = $1 ORDER BY responded_at ASC`,
    [jobId]
  );
  ...
  // Mark selected
  await db.query(
    `UPDATE rfq_broadcast_responses SET selected = TRUE WHERE id = $1`,
    [chosen.id]
  );
  await db.query(
  ...
  // ── Fee event on confirmed match ─────────────────────────────────────────────────
  // Rate is $0.00 now (env var RFQ_MATCH_FEE). Logged as acquisition signal when no wallet.
  const rfqFee = feeService.getRfqMatchFee();
  feeService.logFee({
    event_type:  'rfq_match',
    business_id: chosen.business_id || null,
    rfq_id:      jobId,
    amount_usd:  rfqFee,
    meta: { job_code: job.code, category: job.category, zip: job.zip, biz_name: bizLabel },
  }).catch(e => console.error('[rfqBroadcast] feeService.logFee error:', e.message));
