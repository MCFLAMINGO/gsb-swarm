...
async function broadcastJob({ jobId, code, callerName, category, zip, description }) {
  ...
  if (zip) {
    providers = await db.query(
      `SELECT business_id, name, phone, COALESCE(notification_email, contact_email) AS email, agent_endpoint, agent_key
       FROM businesses
       WHERE status != 'inactive'
         ...
         AND notify_sms = TRUE
         AND (phone IS NOT NULL OR COALESCE(notification_email, contact_email) IS NOT NULL OR agent_endpoint IS NOT NULL)
  ...
  const notifiedProviders = []; // track who was reached for customer confirmation SMS
  ...
  for (const p of providers) {
    ...
    if (smsSent || emailSent) {
      notifiedProviders.push({ name: p.name, phone: p.phone || null, email: p.email || null });
    }
