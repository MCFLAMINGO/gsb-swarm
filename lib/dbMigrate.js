...
async function runStatements(db, sql, label) {
  ...
  for (const stmt of statements) {
    try {
      ...
      // Spurious split artifact: short fragments (e.g. lone `UNIQUE (...)`) that
      // resulted from the line-based splitter slicing inside a multi-line CREATE TABLE.
      // The full CREATE TABLE statement itself is also in `statements` and will
      // succeed (or be idempotently skipped) — these fragments are safe to ignore.
      const trimmedStmt = stmt.trim();
      const isSpuriousSplit =
        e.message.includes('syntax error') && trimmedStmt.length < 80;
      if (
...
async function runMigration() {
  ...
  const dbFiles = fs.readdirSync(DB_DIR)
    ...
    .map(f => ({ filename: f, filepath: path.join(DB_DIR, f) }));

  // Also pick up migrations/NNN_*.sql — applied after db/ files in numeric order
  let migFiles = [];
  if (fs.existsSync(MIGRATIONS_DIR)) {
    migFiles = fs.readdirSync(MIGRATIONS_DIR)
      .filter(f => f.endsWith('.sql'))
      .sort((a, b) => a.localeCompare(b))
      .map(f => ({ filename: f, filepath: path.join(MIGRATIONS_DIR, f) }));
  }

  const allFiles = [...dbFiles, ...migFiles];
  ...
  try {
    ...
    const counts = await Promise.all(tables.map(t =>
      db.query(`SELECT COUNT(*) FROM ${t}`).then(r => `${t}:${(Array.isArray(r) ? r[0] : r.rows?.[0])?.count ?? '?'}`).catch(() => `${t}:?`)
    ));
