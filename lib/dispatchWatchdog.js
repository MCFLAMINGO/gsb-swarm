...
// Absolute max RFQ lifetime — product rule: hit for a day, not a month
const MAX_RFQ_AGE_HOURS      = 24;
...
async function tick() {
  if (_running) return;
  // Skip watchdog during first 3 min of boot — pool is under pressure from migrations
  ...
  try {
    ...
    // Absolute age cap: anything still open after 24h dies (even if deadline_at null/wrong)
    const { rows: agedOut } = await pool.query(`
      SELECT id, category, zip, job_type FROM rfq_requests
      WHERE status = 'open'
        AND created_at < NOW() - ($1::text || ' hours')::interval
      LIMIT 50
    `, [String(MAX_RFQ_AGE_HOURS)]);

    for (const rfq of agedOut) {
      await pool.query(
        `UPDATE rfq_requests SET status = 'expired' WHERE id = $1 AND status = 'open'`,
        [rfq.id]
      );
      await logGap(rfq, 'max_age_24h');
      console.log(`[watchdog] RFQ ${rfq.id} expired — older than ${MAX_RFQ_AGE_HOURS}h`);
    }

    // Same 1-day cap for legacy rfq_jobs so inbox stops resurfacing month-old leads
    await pool.query(`
      UPDATE rfq_jobs
         SET status = 'expired'
...
async function softRetry(rfq, pool) {
  ...
  const cat = (rfq.category || '').toString().trim().toLowerCase();
  const generic = !cat || ['general', 'service', 'services', 'other', 'unknown', 'localbusiness', 'business'].includes(cat);
  if (generic) {
    console.log(`[watchdog] RFQ ${rfq.id} soft-retry skipped — category too broad (${rfq.category || 'null'})`);
    await pool.query(
      `UPDATE rfq_requests SET status = 'no_candidates' WHERE id = $1 AND status = 'open'`,
      [rfq.id]
    );
    return;
  }
