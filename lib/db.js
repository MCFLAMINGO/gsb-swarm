...
function getPool() {
  if (!_pool) {
    ...
    _pool = new Pool({
      ...
      max: 4, // Railway hobby Postgres caps at 25 total — keep headroom for multiple deploys
    ...
    // Kill stale idle connections left by previous deploys so we don't exhaust the 25-connection cap
    _pool.query(`
    ...
    `).then(r => {
      if (r && r.rowCount > 0) console.log(`[db] Cleared ${r.rowCount} stale idle connection(s) on startup`);
    }).catch(() => {}); // non-fatal
  }
  return _pool;
}
