...
function getPool() {
  if (!_pool) {
    ...
    // Workers are background jobs: DB_POOL_MAX=2 (set per-process in index.js).
    // Main server (dashboard-server, localIntelAgent, MCP): DB_POOL_MAX=10 (default).
    // 24 workers × 2 + 1 main × 10 = 58 max possible, but workers are mostly idle = ~15 active peak.
