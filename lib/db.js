...
function getPool() {
  if (!_pool) {
    ...
    // PgBouncer multiplexes 1000 client connections into DEFAULT_POOL_SIZE=20 real Postgres connections.
    // Pool tuning:
    //   max=3   — enough for concurrent inbox+search+workers without hitting the 25-conn Postgres cap.
    //             Previously 2 but idleTimeout=2s caused constant churn that starved inbox requests.
    //   idleTimeout=30s — keep warm connections alive during normal traffic. 2s was too aggressive:
    //             workers cycle rapidly and every new request had to cold-reconnect through PgBouncer.
    //   connectionTimeout=10s — give PgBouncer time to find a free server slot under load.
    ...
    _pool = new Pool({
      ...
      max: poolMax,
      idleTimeoutMillis: 30000,  // keep warm for 30s — avoids cold-connect churn under worker load
      connectionTimeoutMillis: 10000, // give PgBouncer time to find a server slot
      // allowExitOnIdle intentionally omitted — default is false.
      // Setting this true causes the pool to self-destruct when idle, killing all
      // subsequent queries in a long-running Express server. Workers that need clean
      // exit should call db.disconnect() explicitly before process.exit().
    });
    _pool.on('error', (err) => {
