...
function getPool() {
  if (!_pool) {
    ...
    // PgBouncer transaction pooling is live — all connections go through pgbouncer.railway.internal.
    // PgBouncer multiplexes 1000 client connections into backend Postgres connections.
    //
    // Railway PgBouncer env vars to set (in the PgBouncer service, not the app):
    //   DEFAULT_POOL_SIZE=200  — max real Postgres connections PgBouncer opens per db/user pool.
    //                           Railway Pro = 500 max connections; 200 leaves 300 headroom for
    //                           migrations, psql sessions, and Railway's own admin connections.
    //   MAX_CLIENT_CONN=1000   — max simultaneous PgBouncer client connections (app-side).
    //                           1000 workers × DB_POOL_MAX=1 = at most ~50 real clients at once;
    //                           1000 is more than enough and prevents descriptor exhaustion.
    //   POOL_MODE=transaction  — release the backend connection after each transaction, not
    //                           after the client disconnects. Critical for Node.js pools.
    //
    // Each app process only needs DB_POOL_MAX=2 connections max to PgBouncer (not Postgres).
    // The overpass full-ZIP sweep is sequential — it holds 1 connection at a time.
    // SSL is disabled for internal Railway connections (pgbouncer internal hostname, no TLS needed).
    const poolMax = parseInt(process.env.DB_POOL_MAX || '2', 10);
    const isPgBouncer = (url || '').includes('railway.internal');
    _pool = new Pool({
      connectionString: url,
      ssl: isPgBouncer ? false : { rejectUnauthorized: false },
      max: poolMax,
      idleTimeoutMillis: 30000,  // keep connections alive between requests; workers exit via allowExitOnIdle anyway
      connectionTimeoutMillis: 30000, // 30 s — PgBouncer pool=200, Pro=500 conns; boot load needs patience
