...
function getPool() {
  if (!_pool) {
    ...
    // Main server default reduced to 4 — MCP/routing doesn't need 10 simultaneous connections.
    // Workers are forked with DB_POOL_MAX=1. Macro batch runner uses DB_POOL_MAX=1 for all 10 workers.
    // Budget: 4 (main) + ~20 persistent workers×1 + 1 batch runner = ~25, at Railway Hobby cap.
    const poolMax = parseInt(process.env.DB_POOL_MAX || '4', 10);
    _pool = new Pool({
