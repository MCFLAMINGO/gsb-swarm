...
function getPool() {
  if (!_pool) {
    ...
    //   DEFAULT_POOL_SIZE=10   — max real Postgres connections PgBouncer opens per db/user pool.
    //                           10 leaves 15 headroom under the 25-conn Postgres cap for
    //                           migrations, psql sessions, and Railway's own admin connections.
    //   MAX_CLIENT_CONN=200    — max simultaneous PgBouncer client connections (app-side).
    //                           1000 workers × DB_POOL_MAX=1 = at most ~50 real clients at once;
    //                           200 is more than enough and prevents descriptor exhaustion.
    ...
    _pool = new Pool({
      ...
      connectionTimeoutMillis: 10000, // 10 s — gives PgBouncer time to find a backend slot under load
