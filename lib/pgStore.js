...
const SCHEMA = `
  ...
  -- One-time migration for pre-existing rfq_gaps tables created with old schema.
  -- Ensures columns exist (UNIQUE constraint is added separately in ensureSchema)
  ALTER TABLE rfq_gaps ADD COLUMN IF NOT EXISTS vertical TEXT;
  ALTER TABLE rfq_gaps ADD COLUMN IF NOT EXISTS zip      TEXT;
  ALTER TABLE rfq_gaps ADD COLUMN IF NOT EXISTS prompt   TEXT;
  ALTER TABLE rfq_gaps ADD COLUMN IF NOT EXISTS tool     TEXT;
  ALTER TABLE rfq_gaps ADD COLUMN IF NOT EXISTS score    INT;
  ALTER TABLE rfq_gaps ADD COLUMN IF NOT EXISTS needs    TEXT;
  ALTER TABLE rfq_gaps ADD COLUMN IF NOT EXISTS industry TEXT;
  ALTER TABLE rfq_gaps ADD COLUMN IF NOT EXISTS last_ts  TIMESTAMPTZ DEFAULT NOW();
...
async function ensureSchema() {
  ...
  // Workers (DB_POOL_MAX=1) never need to run schema — all tables exist via migrations.
  // Skipping here prevents 18 workers × 1 connection = 18 simultaneous schema checks
  // at boot, which saturates the 25-connection Railway cap.
  if (process.env.DB_POOL_MAX === '1') { _initialized = true; return; }
  // If already running, wait for the in-flight init instead of spawning another
  if (_initPromise) return _initPromise;
  _initPromise = _runSchema();
  return _initPromise;
}

async function _runSchema() {
  try {
    // Short-circuit: if businesses table exists, migrations have already run.
    // Running 44 DDL statements on every cold-start wastes connections and causes
    // pool timeouts when the main process handles concurrent HTTP requests.
    try {
      const exists = await db.query(
        `SELECT 1 FROM information_schema.tables WHERE table_name = 'businesses' LIMIT 1`
      );
      if (exists.length > 0) {
        _initialized = true;
        return;
      }
    } catch (_) { /* fall through to full schema run */ }

    // Split on semicolons, run each statement individually so one failure
    // doesn't block the ALTER TABLE / ADD COLUMN IF NOT EXISTS lines.
    ...
    for (const stmt of statements) {
      try {
        ...
        // Log but continue — ALTER TABLE IF NOT EXISTS errors are non-fatal
        console.warn('[pgStore] Schema stmt warning:', e.message.split('\n')[0]);
      }
    }
    // One-time UNIQUE constraint for rfq_gaps (vertical, zip, prompt) upsert.
    // Required when the table pre-existed with a different PK (e.g. `id`).
    // Runs as a single statement here so the DO/BEGIN/END block isn't split on ';'.
    try {
      await db.query(`
        DO $rfqgaps$ BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM pg_constraint WHERE conname = 'rfq_gaps_natural_key'
          ) AND NOT EXISTS (
            SELECT 1 FROM pg_constraint
             WHERE conrelid = 'rfq_gaps'::regclass AND contype = 'p'
               AND pg_get_constraintdef(oid) ILIKE '%(vertical, zip, prompt)%'
          ) THEN
            BEGIN
              ALTER TABLE rfq_gaps ADD CONSTRAINT rfq_gaps_natural_key UNIQUE (vertical, zip, prompt);
      ...
      console.warn('[pgStore] rfq_gaps unique-constraint migration warning:', e.message.split('\n')[0]);
    }
    _initialized = true;
  } catch (e) {
...
async function appendProbeLog(entry) {
  // Skip during first 3 min of boot — non-fatal, pool is under pressure
...
async function getRouterPatches() {
  // Skip during first 3 min of boot
  if (process.uptime() < 180) return {};
  await ensureSchema();
  if (!db.isReady()) return {};
  try {
...
async function getZipIntelligenceRow(zip) {
  ...
  if (!db.isReady()) return null;
  const row = await db.queryOne(
    'SELECT zip, oracle_json, computed_at, wfh_pct, affluence_pct, population, median_household_income, market_maturity FROM zip_intelligence WHERE zip = $1',
    [zip]
  );
  if (!row) return null;
  return {
    ...
    market_maturity: row.market_maturity || 'established',
...
// All ZIPs in fl_zip_geo — the authoritative statewide ZIP registry (migration 029)
// Used by acsWorker and other workers that need full FL coverage, not just business-coverage ZIPs
async function getAllZipsFromGeo() {
  if (!db.isReady()) return [];
  try {
    const rows = await db.query(
    ...
    // fl_zip_geo may not exist yet (migration 029 pending)
    // Not an error — fallback to FL_ZIP_SEED is intentional during boot
    if (process.env.NODE_ENV !== 'production') console.log('[pgStore] getAllZipsFromGeo fallback:', e.message);
    return [];
  }
}
...
// ── zip_signals — world model materialized signal store ──────────────────────
// Dynamic upsert: pass any subset of zip_signals columns as colsObj.
// Only touches the columns provided — other columns are left untouched.
// colsObj MUST NOT include 'zip' as a key (it is the conflict target).
async function upsertZipSignals(zip, colsObj) {
  if (!db.isReady()) return;
  if (!zip || typeof colsObj !== 'object' || Object.keys(colsObj).length === 0) return;

  // Validate keys (no SQL injection: only allow snake_case alpha/numeric/underscore)
  const safeKey = k => /^[a-z][a-z0-9_]*$/.test(k);
  const entries = Object.entries(colsObj).filter(([k]) => safeKey(k));
  if (!entries.length) return;

  const INT_MAX = 2147483647;
  const safeInt = (v) => (v == null ? null : Math.min(Math.max(Math.round(Number(v)), -INT_MAX), INT_MAX));
  const clampVal = (v) => {
    if (typeof v !== 'number' || !Number.isFinite(v)) return v;
    if (v > INT_MAX || v < -INT_MAX) return safeInt(v);
    return v;
  };

  const cols   = entries.map(([k]) => k);
  const vals   = entries.map(([, v]) => clampVal(v));
  const setClauses = cols.map(c => `${c} = EXCLUDED.${c}`).join(', ');
  const placeholders = vals.map((_, i) => `$${i + 2}`).join(', ');

  try {
    await db.query(
      `INSERT INTO zip_signals (zip, ${cols.join(', ')}, last_updated_at)
    ...
    console.error(`[pgStore] upsertZipSignals(${zip}) error: ${e.message} | cols: ${Object.keys(colsObj).join(',')}`);
    throw e; // re-throw so workers log it too
  }
}
