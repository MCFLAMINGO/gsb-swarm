...
const TREASURY = process.env.LOCAL_INTEL_TREASURY || '0x774f484192Cf3F4fB9716Af2e15f44371fD32FEA';
 ...
 */
async function verifyTransfer(client, txHash, tokenAddr, expectedUSD) {
  try {
...
async function x402Gate(req, toolName, amount, opts = {}) {
  ...
  if (parsed.network === 'tempo') {
    verification = await verifyTransfer(tempoClient(), parsed.txHash, PATHUSD_ADDR, amount);
  } else {
    // Default: Base USDC
    verification = await verifyTransfer(baseClient(), parsed.txHash, BASE_USDC, amount);
  }
  ...
  if (!verification) {
    return {
      ...
      body: {
        ...
        message: `Could not verify $${amount} transfer to ${TREASURY} on ${parsed.network}`,
      },
    };
  }
...
module.exports = { x402Gate, build402, verifyTransfer, TREASURY, BASE_USDC, PATHUSD_ADDR };
