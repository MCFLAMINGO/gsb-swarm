...
const registry = [
  // ── RFQ — non-food service verticals (Step 8) ────────────────────────────
  // Match BEFORE general utility/legacy entries so e.g. "plumber" doesn't fall
  // through to the broad 'restaurant'/'retail' fallbacks. Each entry sets
  // taskClass='RFQ' + resolvesVia='rfq' so router.post('/') dispatches via
  // handleRFQ instead of running a SQL search.
  // ── Home services
  { keywords: ['plumber','plumbing','burst pipe','pipe leak','water leak','drain','clog'],
    taskClass: 'RFQ', group: 'home', category: 'plumber',     resolvesVia: 'rfq' },
  { keywords: ['electrician','electrical','outlet','wiring','circuit breaker','power out'],
    taskClass: 'RFQ', group: 'home', category: 'electrician', resolvesVia: 'rfq' },
  { keywords: ['hvac','air conditioning','ac repair','heating','furnace','duct'],
    taskClass: 'RFQ', group: 'home', category: 'hvac',        resolvesVia: 'rfq' },
  { keywords: ['roofer','roofing','roof repair','roof leak','shingles'],
    taskClass: 'RFQ', group: 'home', category: 'roofer',      resolvesVia: 'rfq' },
  { keywords: ['handyman','handymen'],
    taskClass: 'RFQ', group: 'home', category: 'handyman',    resolvesVia: 'rfq' },
  { keywords: ['painter','painting','exterior paint','interior paint'],
    taskClass: 'RFQ', group: 'home', category: 'painter',     resolvesVia: 'rfq' },
  { keywords: ['landscaper','landscaping','lawn','mow','grass','tree trimming','irrigation'],
    taskClass: 'RFQ', group: 'home', category: 'landscaper',  resolvesVia: 'rfq' },
  { keywords: ['cleaner','cleaning','house cleaning','maid','janitorial'],
    taskClass: 'RFQ', group: 'home', category: 'cleaner',     resolvesVia: 'rfq' },
  // ── Auto services
  { keywords: ['mechanic','auto repair','car repair','oil change','tire','brake','transmission'],
    taskClass: 'RFQ', group: 'auto', category: 'mechanic',    resolvesVia: 'rfq' },
  { keywords: ['tow truck','towing','tow my car','roadside assistance','jump start'],
    taskClass: 'RFQ', group: 'auto', category: 'towing',      resolvesVia: 'rfq' },

  // ── Reservation routing (must precede temporal/cuisine/catch-all) ────────
  // Caught BEFORE 'dinner reservation' temporal entry and the 'restaurant'
  // catch-all so any reservation phrasing routes through handleReservation
  // (channel-aware: web=phone+website, voice=<Dial>, SMS=text).
  { keywords: ['reservation', 'make a reservation', 'book a table', 'book a reservation',
               'table for', 'reserve a table', 'get a table', 'reserve at', 'reservation at',
               'can i get a reservation', 'do you take reservations'],
    taskClass: 'RESERVATION', group: 'food', category: 'restaurant', resolvesVia: 'reservation' },
  ...
  // ── Dessert / ice cream / frozen treats (DISCOVER · food) ────────────────
  // Must precede the catch-all 'restaurant' entry so dessert keywords resolve
  // to category 'dessert' instead of falling through to generic food.
  { keywords: ['ice cream', 'gelato', 'frozen yogurt', 'frozen custard', 'dessert', 'sweet treat',
               'sundae', 'milkshake', 'milk shake', 'custard', 'sorbet', 'soft serve',
               'dq', 'dairy queen', "flo's", 'culvers', "culver's", "kilwin's", "bruster's",
               'marble slab', 'carvel', 'baskin', 'cold stone', "rita's", 'sugar cone', 'cone'],
    taskClass: 'DISCOVER', group: 'food', category: 'dessert', resolvesVia: 'search' },
  ...
  // ── Healthcare / medical (all DISCOVER — never RFQ) ───────────────────────
  { keywords: ['plastic surgeon', 'plastic surgery', 'cosmetic surgery', 'rhinoplasty', 'nose job',
               'botox', 'filler', 'lip filler', 'liposuction', 'tummy tuck', 'facelift',
               'breast implant', 'mommy makeover', 'coolsculpting', 'body contouring'],
    taskClass: 'DISCOVER', group: 'health', category: 'plastic_surgery', resolvesVia: 'search' },
  { keywords: ['dermatologist', 'dermatology', 'skin clinic', 'skin care', 'laser hair', 'laser treatment'],
    taskClass: 'DISCOVER', group: 'health', category: 'dermatology',  resolvesVia: 'search' },
  { keywords: ['med spa', 'medspa', 'medical spa', 'aesthetics', 'aesthetic clinic'],
    taskClass: 'DISCOVER', group: 'health', category: 'medical_spa',  resolvesVia: 'search' },
  { keywords: ['dentist', 'dental', 'orthodontist', 'braces', 'teeth', 'dentures', 'denture', 'prosthodont'],
    taskClass: 'DISCOVER', group: 'health', category: 'dentist',      resolvesVia: 'search' },
  { keywords: ['doctor', 'physician', 'primary care', 'family doctor', 'urgent care', 'clinic'],
    taskClass: 'DISCOVER', group: 'health', category: 'clinic',       resolvesVia: 'search' },
  { keywords: ['hospital', 'emergency room', 'er ', 'emergency care'],
    taskClass: 'DISCOVER', group: 'health', category: 'hospital',     resolvesVia: 'search' },
  { keywords: ['optometrist', 'eye doctor', 'eye care', 'vision', 'glasses', 'contacts'],
    taskClass: 'DISCOVER', group: 'health', category: 'optician',     resolvesVia: 'search' },
  ...
  // ── Jewelry / fine jewelry / watches (DISCOVER · retail) ──────────────────
  // Includes named FL jeweler "Underwood's" (Ponte Vedra). Must precede generic
  // retail fallback so jewelry queries resolve to category 'jewelry'.
  { keywords: ['jewelry', 'jeweler', 'jewellery', 'jewellers', 'ring', 'diamond', 'necklace',
               'bracelet', 'earrings', 'watch', 'engagement ring', 'wedding band', 'pendant',
               'gold', 'silver', 'gemstone', 'underwood', "underwood's", 'fine jewelry',
               'custom jewelry', 'jewelry store', 'jewel'],
    taskClass: 'DISCOVER', group: 'retail', category: 'jewelry',     resolvesVia: 'search' },
  { keywords: ['atm', 'bank'],
