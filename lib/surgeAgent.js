...
async function fetchMenu(businessId) {
  // Try to get wallet address from businesses.wallet for public read
  // (no APIM key needed — Surge allows public inventory read via x-wallet header)
  let walletAddr = null;
  try {
    const [reg] = await db.query(
      `SELECT wallet AS deposit_address FROM businesses WHERE business_id = $1 LIMIT 1`,
      [businessId]
    );
...
async function createOrder(businessId, items, jurisdictionCode = 'US-FL') {
  ...
  // Also pass wallet address so Surge can resolve merchant by wallet, not name
  let walletAddr = null;
  try {
    const [reg] = await db.query(
  ...
  if (walletAddr) headers['x-wallet'] = walletAddr;
