...
async function migrate() {
  if (migrated) return;
  // Auto-add choices column for existing tables
  await db.query(`ALTER TABLE voice_sessions ADD COLUMN IF NOT EXISTS choices JSONB`).catch(() => {});
  await db.query(`
  ...
  if (!existing) {
    ...
    await db.query(
      `INSERT INTO voice_sessions
         (call_sid, caller_phone, stage, business_id, business_name, zip, category, cart, choices)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
      [s.call_sid, s.caller_phone, s.stage, s.business_id,
       s.business_name, s.zip, s.category, s.cart,
       patch.choices ? JSON.stringify(patch.choices) : null]
    );
  } else {
    ...
    const vals   = [];
    let i = 1;
    const allowed = ['stage','business_id','business_name','zip','category','cart','caller_phone','choices'];
    for (const key of allowed) {
      if (patch[key] !== undefined) {
        fields.push(`${key} = $${i++}`);
        vals.push((key === 'cart' || key === 'choices') ? JSON.stringify(patch[key]) : patch[key]);
      }
    }
...
/**
 * Find the most recent surge_pending session for a caller phone.
 * Used when an SMS 1/2/3 reply comes in after the call has ended.
 */
async function getSurgePending(callerPhone) {
  await migrate();
  const rows = await db.query(
    `SELECT * FROM voice_sessions
      WHERE caller_phone = $1
        AND stage = 'surge_pending'
...
async function clearByPhone(callerPhone) {
  await migrate();
  await db.query(
    `DELETE FROM voice_sessions WHERE caller_phone = $1 AND stage = 'surge_pending'`,
    [callerPhone]
  );
}
...
module.exports = { get, save, addToCart, clearSession, getSurgePending, clearByPhone, cartTotal, cartSummary };
