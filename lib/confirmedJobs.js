...
async function notifyBusiness(job, biz) {
  // SMS
  if (biz.phone && process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN && process.env.TWILIO_PHONE_NUMBER) {
    try {
      const twilio = require('twilio')(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
      await twilio.messages.create({
        from: process.env.TWILIO_PHONE_NUMBER,
        to:   biz.phone,
        body: buildSmsBody(job, biz.name),
      });
      console.log(`[confirmedJobs] SMS sent to ${biz.phone} for job ${job.id}`);
    } catch (e) {
