...
async function createRfq({
  ...
  async function queryMatchedBusinesses(radius) {
    const catFilter = category ? `AND category ILIKE $CAT` : '';
    const zipFilter = zip      ? `AND zip = $ZIP`         : '';
    ...
    params.push(limit);
    const { rows } = await pool.query(
      `SELECT business_id, name, notification_email, dispatch_token, lat, lon, NULL AS distance_miles
         FROM businesses
  ...
  console.log(`[rfqService] createRfq rfq_id=${rfq_id} matched=${matched_count} notified=${notified_count} autonomy=${autonomy} job_type=${job_type}${warning ? ' WARN:'+warning : ''}`);

  return { rfq_id, matched_count, notified_count, map_url: map_url || null, ...(warning ? { warning } : {}) };
}
