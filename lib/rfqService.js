...
async function declineResponse(rfq_id, response_id, reason) {
  ...
  await pool.query(
    `UPDATE rfq_responses SET status = 'declined' WHERE id = $1 AND rfq_id = $2`,
    [response_id, rfq_id]
  );

  // If there was a booking for this response, cancel it and reopen RFQ
  const { rows: [booking] } = await pool.query(
    `SELECT id FROM rfq_bookings WHERE rfq_id = $1 AND response_id = $2 LIMIT 1`,
    [rfq_id, response_id]
  );
  if (booking) {
    await pool.query(`DELETE FROM rfq_bookings WHERE id = $1`, [booking.id]);
    await pool.query(`UPDATE rfq_requests SET status = 'open' WHERE id = $1`, [rfq_id]);
  }
