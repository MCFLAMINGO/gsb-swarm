...
//       Any future notification string must be added here in both languages
//       before being used anywhere in the codebase.
const NOTIF_STRINGS = {
  verify_subject: {
    en: 'Your LocalIntel verification code',
    es: 'Tu código de verificación de LocalIntel',
  },
  verify_sms: {
    en: (code) => `LocalIntel code: ${code}. Expires in 30 min.`,
    es: (code) => `Código LocalIntel: ${code}. Expira en 30 min.`,
  },
  welcome_subject: {
    en: (name) => `Welcome to LocalIntel — ${name}`,
    es: (name) => `Bienvenido a LocalIntel — ${name}`,
  },
  welcome_body: {
    en: "Your listing is claimed. You'll receive market intelligence when agents query your area.",
    es: 'Tu perfil está reclamado. Recibirás inteligencia de mercado cuando los agentes consulten tu área.',
  },
  welcome_cta: {
    en: 'View Dashboard',
    es: 'Ver panel',
  ...
  job_subject: {
    en: (cat, zip) => `New job request — ${cat} in ${zip}`,
    es: (cat, zip) => `Nueva solicitud de trabajo — ${cat} en ${zip}`,
  },
  job_body: {
    en: 'A new job request has been posted in your area. Open your inbox to respond.',
    es: 'Se publicó una nueva solicitud de trabajo en tu área. Abre tu bandeja para responder.',
  },
  job_cta: {
    en: 'View Request →',
    es: 'Ver solicitud →',
  ...
  intel_subject: {
    en: (zip) => `Market update for ${zip}`,
    es: (zip) => `Actualización de mercado para ${zip}`,
  },
  intel_body: {
    en: 'New market intelligence is available for your area. Log in to view.',
    es: 'Nueva inteligencia de mercado disponible para tu área. Ingresa para ver.',
  },
  unsubscribe: {
    en: 'unsubscribe',
...
// Helper: resolve a string or function from NOTIF_STRINGS by key + lang + optional args
function ns(key, lang = 'en', ...args) {
...
// Fetch owner_language for a business_id
async function getOwnerLang(business_id) {
  try {
    const [row] = await db.query(
      `SELECT owner_language FROM businesses WHERE business_id = $1 LIMIT 1`,
      [business_id]
    );
    return (row?.owner_language === 'es') ? 'es' : 'en';
  } catch { return 'en'; }
}
...
async function deliverSms(notification) {
  ...
  const biz = await db.query(
    `SELECT notification_phone, name, owner_language FROM businesses WHERE business_id = $1`,
    [notification.business_id]
  );
  if (!biz[0]?.notification_phone) throw new Error('no phone on file');

  const lang    = (biz[0].owner_language === 'es') ? 'es' : 'en';
  ...
  // Use bilingual body if payload has a type hint, otherwise fall back to raw subject/body
  const bodyText = notification.payload?.body_es && lang === 'es'
    ? notification.payload.body_es
    : (notification.payload?.body || notification.subject || '');
  const prefix = lang === 'es' ? 'LocalIntel' : 'LocalIntel';
  const text = `${prefix}: ${bodyText}`.slice(0, 160);
...
async function deliverEmail(notification) {
  ...
  const biz = await db.query(
    `SELECT notification_email, name, owner_language FROM businesses WHERE business_id = $1`,
    [notification.business_id]
  );
  ...
  // Use bilingual body/subject if available in payload
  const subject = (lang === 'es' && p.subject_es) ? p.subject_es : (notification.subject || 'LocalIntel Update');
  const body    = (lang === 'es' && p.body_es)    ? p.body_es    : (p.body || '');
  const ctaLabel = (lang === 'es' && p.cta_label_es) ? p.cta_label_es : (p.cta_label || (lang === 'es' ? 'Ver detalles' : 'View Details'));
  const unsubLabel = lang === 'es' ? 'cancelar suscripción' : 'unsubscribe';
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:24px">
      ...
      <div style="margin-top:32px;font-size:12px;color:#9CA3AF">
        LocalIntel · <a href="https://thelocalintel.com/unsubscribe?id=${notification.business_id}" style="color:#9CA3AF">${unsubLabel}</a>
      </div>
    </div>`;
  ...
  console.log(`[notify/email] sent to ${biz[0].notification_email} (${lang}) — id: ${result.data?.id}`);
}
