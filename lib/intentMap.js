'use strict';
const catMap = require('./categoryMap');
...
function resolveIntent(text) {
  ...
  // 3. categoryMap fallback — covers all paths (voice, SMS, web, MCP) from single source
  const cmResult = catMap.resolve(raw);
  if (cmResult) {
    return { cat: cmResult.cat, tags: null, deflect: false, confidence: 'keyword' };
  }
