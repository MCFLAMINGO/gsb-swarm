...
// ── Follow-up state (Postgres-backed, 10-min TTL) ──────────────────────────
// Persisted in task_followup_sessions so state survives Railway restarts and is
// consistent across proxy hops where sessionId (x-forwarded-for) may vary.
const db = require('./db');

const FOLLOWUP_TTL_MS = 10 * 60 * 1000; // 10 minutes

async function getTaskFollowUp(sessionId) {
  if (!sessionId) return null;
  try {
    const rows = await db.query(
      `SELECT state FROM task_followup_sessions
    ...
    console.error('[taskFollowUp] getTaskFollowUp error:', err.message);
    return null;
  }
}

async function setTaskFollowUp(sessionId, state) {
  if (!sessionId || !state) return;
  const expiresAt = new Date(Date.now() + FOLLOWUP_TTL_MS);
  try {
    await db.query(
      `INSERT INTO task_followup_sessions (session_id, state, expires_at)
       VALUES ($1, $2, $3)
       ON CONFLICT (session_id) DO UPDATE
         SET state = EXCLUDED.state,
             expires_at = EXCLUDED.expires_at,
             created_at = NOW()`,
      [`task:${sessionId}`, JSON.stringify(state), expiresAt]
    );
  } catch (err) {
    console.error('[taskFollowUp] setTaskFollowUp error:', err.message);
  }
}

async function clearTaskFollowUp(sessionId) {
  if (!sessionId) return;
  try {
    await db.query(
      `DELETE FROM task_followup_sessions WHERE session_id = $1`,
      [`task:${sessionId}`]
    );
  } catch (err) {
    console.error('[taskFollowUp] clearTaskFollowUp error:', err.message);
  }
}

setInterval(() => {
  db.query(`DELETE FROM task_followup_sessions WHERE expires_at < NOW()`)
    .catch(err => console.error('[taskFollowUp] cleanup error:', err.message));
}, 5 * 60 * 1000);

// ── FL city abbreviations → full name ──────────────────────────────────────
// Used to enrich follow-up messages when user mentions a city by abbrev.
const FL_CITY_ABBREV = {
  JAX: 'Jacksonville',
  PVB: 'Ponte Vedra Beach',
  STJ: 'St. Augustine',
  STA: 'St. Augustine',
  NOC: 'Nocatee',
  OPK: 'Orange Park',
  ...
  ORL: 'Orlando',
  MCO: 'Orlando',
  ALT: 'Altamonte Springs',
};

// Build a regex that matches any abbreviation as a whole word (case-insensitive).
const FL_CITY_ABBREV_RE = new RegExp(
  `\\b(${Object.keys(FL_CITY_ABBREV).join('|')})\\b`,
  'i'
);

function _detectFLCity(text) {
  if (!text) return null;
  const m = FL_CITY_ABBREV_RE.exec(text);
  if (!m) return null;
  const key = m[1].toUpperCase();
  return FL_CITY_ABBREV[key] || null;
}

// Human-readable category labels for multi-task overviews.
const _CAT_LABELS = {
  dry_cleaning: 'dry cleaning',
  pharmacy:     'pharmacy',
  grocery:      'grocery',
  pet_services: 'pet supplies',
...
function _catLabel(cat) { return _CAT_LABELS[cat] || cat || 'errand'; }
...
// NOTE: "i need a plumber/handyman/electrician" used to be in this list but is
// now recognized as a send_someone service request (see SERVICE_REQUEST_RE).
const DISCOVERY_HINT_RE =
  /\b(where\s+(?:is|can\s+i|are|do\s+i|should\s+i)|find\s+(?:me\s+)?a|search\s+for|look\s+up|recommend(?:ation)?|suggest|hours\s+of|phone\s+(?:number\s+)?for|address\s+(?:of|for)|best\s+(?:place|spot|restaurant|dry\s+cleaner|cafe|pharmacy|grocery|store)|near(?:est|by)\s+(?:dry|laun|pharm|groc|resto|rest)|i\s+need\s+a\s+(?:cleaner|dry|laun|pharm|groc|vet|doctor|dentist|lawyer|barber|salon)|pickup\s+truck|pickup\s+basketball|pickup\s+game|pickup\s+(?:locations?|spots?|points?|service|option)|food\s+pickup\s+(?:locations?|options?|near)|order\s+(?:online|ahead)|delivery\s+(?:restaurant|service|option|near)|restaurants?\s+(?:that\s+deliver|near|in|open)|what\s+(?:pharmacy|restaurant|store|cafe|cleaner|grocery)|who\s+delivers|can\s+you\s+recommend)\b/i;
...
// Service dispatch — user wants a tradesperson sent out.
// "I need a plumber" / "get me a handyman" / "send someone to fix my AC"
const SERVICE_REQUEST_RE =
  /\b(?:i\s+need|get\s+(?:me|us)|send\s+(?:me|us)?)\s+(?:a\s+|an\s+)?(plumber|handyman|handy\s+man|electrician|hvac|ac\s+(?:tech|guy|repair|repair\s+man)|locksmith|pest\s+control|exterminator|cleaning\s+service|maid|housekeeper|painter|landscaper|lawn\s+(?:guy|service)|pool\s+(?:guy|service|cleaner)|roofer|appliance\s+repair|mover|movers)\b/i;
...
// "bring me X" — delivery request (pickup)
const BRING_ME_RE =
  /\bbring\s+(?:me|us|my|our)\s+\w/i;

// "bring the/this/that X to Y" — dropoff
const BRING_TO_RE =
  /\bbring\s+(?:the|this|that|it|them)\s+\w[\w\s]{0,30}\s+to\s+\w/i;
...
const CAT_PATTERNS = [
  ...
  // ── Pharmacy / prescriptions / medical pickup ────────────────────────────
  // Includes: rx, meds, scripts, pills, CVS, Walgreens, Winn-Dixie/Publix
  // pharmacy. Also NE FL hospital/health systems and lab pickups.
  // B9: added "medication/prescription/meds/rx picked up" and the inverse
  // "pick up ... prescription/meds" so "i need medication picked up at cvs"
  // intercepts here instead of falling into the ordering flow.
  {
    re: /\b(prescription|prescriptions|meds|medication|medications|medicine|medicines|pharmacy|pharmacies|rx\b|refill|scripts?|pills?|capsules?|tablets?|dose|dosage|cvs|walgreens|rite\s*aid|winn-?dixie\s+pharm|publix\s+pharm|labs?\b|lab\s+results?|lab\s+work|bloodwork|blood\s+work|mayo(?:\s+clinic)?|baptist(?:\s+health|\s+medical)?|st\.?\s+vincent'?s|ascension|uf\s+health|memorial\s+hospital|orange\s+park\s+medical|(?:medication|medications|prescription|prescriptions|meds|rx)\s+picked\s+up|pick\s+up.*(?:prescription|medication|meds?|rx))\b/i,
    cat: 'pharmacy',
    followUp: 'Which pharmacy or clinic? (reply with name or NONE)',
    followUpKey: 'venue_name',
  },

  // ── Schools (NE FL) — pickup/dropoff at school is errands ────────────────
  // Matches school name OR generic "school" / "kids from school".
  {
    re: /\b(nease(?:\s+high)?|allen\s+d\.?\s+nease|palm\s+valley\s+academy|ponte\s+vedra\s+high|pedro\s+menendez|creekside\s+high|bartram\s+trail(?:\s+high)?|fletcher\s+(?:high|middle)|sandalwood\s+high|atlantic\s+coast\s+high|stanton\s+(?:college|prep)|paxon\s+(?:school)?|the\s+bolles\s+school|episcopal\s+school|providence\s+school|school\s+pickup|kids?\s+from\s+(?:school|the\s+school)|daughter\s+from\s+(?:school|the\s+school|[A-Z])|son\s+from\s+(?:school|the\s+school|[A-Z])|hospital|the\s+hospital|er\b|emergency\s+room|the\s+er)\b/i,
    cat: 'errands',
    followUp: 'Where exactly should the pickup/dropoff happen? (reply with address or building name)',
    followUpKey: 'venue_name',
  },
  ...
  // Must precede 'restaurant' so dessert-specific queries don't fall through
  // to the generic food category. FL brands: Flo's, Dairy Queen, Kilwin's,
  // Culver's, Cold Stone, Marble Slab, Bruster's, Carvel, Baskin-Robbins.
  {
    re: /\b(ice[\s-]?cream|gelato|frozen\s+yogurt|frozen\s+custard|soft[\s-]?serve|sundae|milk[\s-]?shake|custard|sorbet|sweet\s+treat|dessert|Dairy\s+Queen|DQ\b|Flo'?s|Culver'?s|Kilwin'?s|Marble\s+Slab|Cold\s+Stone|Baskin|Carvel|Bruster'?s)\b/i,
    cat: 'dessert',
    followUp: 'Which ice cream or dessert spot? (reply with name or NONE)',
    followUpKey: 'venue_name',
  },
  ...
  // ── Auto / car / AC + HVAC ───────────────────────────────────────────────
  {
    re: /\b(car|vehicle|truck|van|suv|auto|my\s+ride|the\s+car|the\s+truck|my\s+car|my\s+truck|my\s+vehicle|ac\b|a\/c\b|air\s+condition(?:er|ing)?|hvac|heater|furnace)\b/i,
    cat: 'auto_repair',
    followUp: 'Which shop or service? (reply with name or NONE)',
    followUpKey: 'venue_name',
  },
  ...
  // Named FL jeweler "Underwood's" (Ponte Vedra) baked in. Engagement rings,
  // wedding bands, diamonds, etc.
  {
    ...
    cat: 'jewelry',
    followUp: 'Which jewelry store? (reply with name or NONE)',
    followUpKey: 'venue_name',
  },
...
function _firstVerbMatch(text) {
  const checks = [
    { re: SERVICE_REQUEST_RE, type: 'send_someone' },
...
function detectTaskIntent(text) {
  ...
  // Detect FL city abbreviation (JAX, PVB, NOC, etc.) for follow-up enrichment.
  const city = _detectFLCity(raw);
  ...
  if (primaryCat) {
    let followUp = city
      ? `${primaryCat.followUp.replace(/\?\s*\(/, `? (in ${city}, `)}`
      : primaryCat.followUp;
    // When 3+ tasks are detected, prepend an overview enumerating them.
    if (allTasks.length >= 3) {
      const labels = allTasks.map(t => _catLabel(t.cat));
      followUp = `I see ${allTasks.length} tasks: ${labels.join(', ')}. Let's start with ${labels[0]} — ${followUp}`;
    }
    return {
      isTask: true,
      taskType,
  ...
  return {
    ...
    cat: 'errands',
    followUp: city
      ? `What do you need picked up or dropped off in ${city}? (reply with details or CANCEL)`
      : 'What do you need picked up or dropped off, and where? (reply with details or CANCEL)',
