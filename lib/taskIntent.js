...
const CAT_PATTERNS = [
  ...
  {
    re: /\b(prescription|prescriptions|meds|medication|medications|medicine|medicines|pharmacy|pharmacies|rx\b|refill|scripts?|pills?|capsules?|tablets?|dose|dosage|cvs|walgreens|rite\s*aid|winn-?dixie\s+pharm|publix\s+pharm|labs?\b|lab\s+results?|lab\s+work|bloodwork|blood\s+work|mayo(?:\s+clinic)?|baptist(?:\s+health|\s+medical)?|st\.?\s+vincent'?s|ascension|uf\s+health|memorial\s+hospital|orange\s+park\s+medical)\b/i,
    cat: 'pharmacy',
