...
const DISCOVERY_HINT_RE =
  /\b(where\s+(?:is|can\s+i|are|do\s+i|should\s+i)|find\s+(?:me\s+)?a|search\s+for|look\s+up|recommend(?:ation)?|suggest|hours\s+of|phone\s+(?:number\s+)?for|address\s+(?:of|for)|best\s+(?:place|spot|restaurant)|near(?:est|by)\s+(?:dry|laun|pharm|groc|resto|rest)|i\s+need\s+a\s+(?:cleaner|dry|laun|pharm|groc|mechanic|plumber|vet|doctor|dentist|lawyer|barber|salon))\b/i;
...
const CAT_PATTERNS = [
  ...
  // ── Flowers / gifts ──────────────────────────────────────────────────────
  {
    re: /\b(flowers?|bouquet|arrangement|roses?|floral|florist|gift\s+basket|gift\s+card|birthday\s+(?:cake|gift)|1-800-flowers?)\b/i,
    cat: 'florist',
