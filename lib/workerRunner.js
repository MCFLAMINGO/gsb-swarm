...
const DAEMON_WORKERS = [
  ...
  'cesWorker',
  'qcewWorker',
  'schoolEnrollmentWorker',
  'countyPermitsWorker',
  'worldModelWorker',
  'businessSignalWorker',
  'websiteEnricherWorker',
];
...
const WORKER_GROUPS = {
  ...
  data_ingestion:    ['overpassWorker', 'acsWorker', 'censusLayerWorker', 'oceanFloorWorker', 'fdotWorker', 'websiteEnricherWorker'],
  infrastructure:    ['zipBriefWorker', 'zipCoordinatorWorker', 'waveSurfaceWorker', 'surfaceCurrentWorker', 'oracleWorker'],
  // B66 — manual-trigger outbound outreach to unclaimed businesses (SMS + email).
  // NOT in DAEMON_WORKERS — never auto-fired on deploy.
  outreach:          ['claimOutreachWorker'],
};
...
function runWorker(workerName, extraEnv) {
