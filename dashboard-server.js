...
app.get('/api/compute/test', async (req, res) => {
  ...
  const agents = [[1332,'CEO'],[1333,'WalletProfiler'],[1334,'AlphaScanner'],[1335,'TokenAnalyst'],[1336,'ThreadWriter']];
  const entries = await Promise.all(agents.map(async ([id, name]) => {
    const key = getAgentKey(id);
    if (!key) return [name, 'NO KEY IN RAILWAY'];
    try {
      ...
      return [name, r.ok ? `✅ ${j?.choices?.[0]?.message?.content?.trim() || 'responded'}` : `❌ HTTP ${r.status}: ${JSON.stringify(j).slice(0,120)}`];
...
(function launchLocalIntelWorkers() {
  ...
  // [ISOLATED-TEST] { name: 'LODES Jobs Worker',             file: 'workers/lodesWorker.js'         },  // 7-day freshness check (built-in)
