...
router.get('/.well-known/mcp/server-card.json', (req, res) => {
  res.json({
    serverInfo: { name: 'LocalIntel by MCFLAMINGO', version: '1.0.0', description: 'Agentic business intelligence for St. Johns County FL. 12 MCP tools. $0.01–$0.05/call.' },
    ...
    tools: [
      ...
      { name: 'local_intel_corridor',  description: 'Businesses along a named street corridor (e.g. A1A, Palm Valley Road).', inputSchema: { type: 'object', required: ['corridor'], properties: { corridor: { type: 'string', description: 'Street or corridor name', examples: ['A1A', 'Palm Valley Road'] }, zip: { type: 'string', description: 'Optional ZIP filter' } } } },
