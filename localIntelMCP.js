 ...
 * and call tools to query the LocalIntel Florida dataset (1,473 ZIPs statewide).
 *
 * Tools exposed:
...
const TOOLS = {
  ...
  local_intel_retail:       { fn: (p) => handleVerticalQuery('retail',       p.query || 'give me a retail market overview', p.zip), desc: 'Retail market intelligence: store categories, spending capture, consumer profile, undersupplied niches.' },
  local_intel_construction: { fn: (p) => handleVerticalQuery('construction', p.query || 'give me a construction market overview', p.zip), desc: 'Construction and home services intelligence: active permits, contractor density, population growth driving demand.' },
  ...
  local_intel_query:        { fn: handleQuery, desc: 'Fuzzy intent router — pass any plain-English question about any local market. Automatically detects ZIP, industry vertical, and best tool. Checks inference cache first (instant return if hit). On cache miss, routes to the correct vertical agent, stores result, and dispatches a scout agent if confidence is low. Handles region queries ("Tampa", "Miami-Dade", "Central Florida") by evaluating top ZIPs in parallel. Best first call for any LLM that does not know the ZIP or industry in advance.' },
    ...
    return {
      ...
      result: {
        prompts: [
          ...
          { name: 're_014_im_a_buyer_relocating_from', description: '[Realtor] I\'m a buyer relocating — what\'s the income and lifestyle profile of this ZIP and is it right for a family with school-age children?', arguments: [{ name: 'zip', description: 'ZIP code (e.g. 33629, 32801, 33179)', required: false, default: '33629' }] },
