 ...
 * ── DATASETS ─────────────────────────────────────────────────────────────────
 *
 * LAYER A: BFS — Business Formation Statistics (monthly, county-level)
 *   Source:  api.census.gov/data/timeseries/bfs
 *   Vintage: current month (1-2 month lag)
 *   Geo:     County FIPS (all 67 FL counties)
 *   Writes:  macro_indicators (source='bfs') + zip_signals.macro_bfs_apps_latest
 *   Why:     Leading indicator — new business applications precede actual
 *            business formation by 6-18 months. High-propensity apps signal
 *            real entrepreneurial intent. Oracle uses this to score ZIP momentum.
...
const BFS_TTL_DAYS    = 28;          // re-fetch BFS monthly
...
// ── LAYER A: BFS — Business Formation Statistics ──────────────────────────────
async function ingestBFS() {
  console.log('[censusMacro] BFS: fetching business formation statistics...');
  let ingested = 0;
  let skipped  = 0;

  for (const { name, fips } of FL_COUNTIES) {
    const geoId = `${FL_STATE_FIPS}${fips}`;
    const latest = await getLatestPeriod('bfs', geoId);

    // BFS data has ~2 month lag. Check if we have data from this month already.
    const nowPeriod = new Date().toISOString().slice(0, 7); // YYYY-MM
    if (latest && latest >= nowPeriod) {
      skipped++;
      continue;
    }
    ...
    try {
      ...
      const headers = raw[0];
      const idxBA   = headers.indexOf('BA_BA');
      const idxHBA  = headers.indexOf('BA_HBA');
      const idxWBA  = headers.indexOf('BA_WBA');
      const idxYear = headers.indexOf('YEAR');
      const idxMon  = headers.indexOf('MONTH');

      // Upsert each month row
      for (let i = 1; i < raw.length; i++) {
        const row    = raw[i];
        const period = `${row[idxYear]}-${String(row[idxMon]).padStart(2, '0')}`;
        const metrics = {
          ba_total:      toN(row[idxBA]),
          ba_highprop:   toN(row[idxHBA]),
          ba_wba:        toN(row[idxWBA]),
        };
        ...
        await db.query(
          `INSERT INTO macro_indicators (source, geo_type, geo_id, geo_name, period, metrics, vintage, fetched_at)
           ...
           ON CONFLICT (source, geo_id, period) DO UPDATE SET
             ...
             fetched_at = NOW()`,
          ['bfs', 'county', geoId, `${name} County, FL`, period, JSON.stringify(metrics), '2022-present']
        );
      }

      // Stamp latest month onto every ZIP in this county (for oracle/MCP no-join access)
      const latestRow = raw[raw.length - 1];
      const latestPeriod = `${latestRow[idxYear]}-${String(latestRow[idxMon]).padStart(2, '0')}`;
      const latestMetrics = {
        ba_total:    toN(latestRow[idxBA]),
        ba_highprop: toN(latestRow[idxHBA]),
        ...
        await db.query(
          `INSERT INTO zip_macro_signals (zip, bfs_county_apps_total, bfs_county_apps_highprop, bfs_county_wba, bfs_county_period, bfs_updated_at, updated_at)
           ...
           ON CONFLICT (zip) DO UPDATE SET
             ...
             bfs_updated_at           = NOW(),
             updated_at               = NOW()`,
          [zip, latestMetrics.ba_total, latestMetrics.ba_highprop, latestMetrics.ba_wba, latestPeriod]
        );
      ...
      console.error(`[censusMacro] BFS failed for ${name}:`, err.message);
    }
  }

  console.log(`[censusMacro] BFS: ${ingested} counties ingested, ${skipped} skipped (fresh)`);
}
