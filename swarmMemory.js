...
function readNarrative(symbol) {
  ...
  if (!n) return null;
  if (Date.now() - n.updatedAt > TTL_MS) return null; // expired
  return n;
}
...
function readFinding(key) {
  ...
  if (!f) return null;
  if (Date.now() - f.createdAt > TTL_MS) return null;
  return f;
}
...
function getPinnedContext() {
  const mem = load();
  const now = Date.now();
  return mem.pinnedContext.filter(p => now - p.createdAt < TTL_MS);
}
...
function listNarratives() {
  ...
  return Object.values(mem.narratives)
    .filter(n => now - n.updatedAt < TTL_MS)
...
function prune() {
  ...
  for (const k of Object.keys(mem.narratives)) {
    if (now - mem.narratives[k].updatedAt > TTL_MS) delete mem.narratives[k];
  }
  for (const k of Object.keys(mem.agentFindings)) {
    if (now - mem.agentFindings[k].createdAt > TTL_MS) delete mem.agentFindings[k];
  }
  mem.pinnedContext = mem.pinnedContext.filter(p => now - p.createdAt < TTL_MS);
  save(mem);
}
